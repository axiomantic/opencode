...
You can create multiple configurations of the same provider using the `extends` field. This is useful when you need to use the same provider with different credentials, such as separate work and personal accounts.
...
3. **Options merge** - The `options` you specify (like `apiKey`) are applied to the extended provider
4. **Independent provider** - The extended provider appears as a separate entry in the model picker

:::tip
Both the base provider and extended providers can coexist. If you have `ANTHROPIC_API_KEY` set and also define `anthropic-work`, you'll see both in the model picker.
:::
...
| Option | Description |
|--------|-------------|
| `extends` | The provider ID to extend from (e.g., `"anthropic"`, `"openai"`) |
