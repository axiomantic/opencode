...
Use the `extends` field to create multiple configurations of the same provider with different credentials. This is useful for managing work and personal accounts separately.
...
- `extends` - The base provider ID to inherit models from
- `name` - Display name for the provider in the model picker

The extended provider will have all models from the base provider available under the new provider ID (e.g., `anthropic-work/claude-sonnet-4-20250514`).

[Learn more about provider profiles](/docs/providers#provider-profiles).
